...
const dataParamsTable = function (method = 'GET') {
    return {
        type: method,
        url: '/ServicesCategoryManage/GetList',
        data: function (d) {
...
function ShowEditModal(elm, id) {
    ...
    $.get(`/ServicesCategoryManage/P_Edit/${id}`).done(function (response) {
...
function ShowViewModal(elm, id) {
    let text = $(elm).html();
    $(elm).attr('disabled', true); $(elm).html(_loadAnimationSmallHtml);
    $.get(`/ServicesCategoryManage/P_View/${id}`).done(function (response) {
        $(elm).attr('disabled', false); $(elm).html(text);
...
function Delete(id) {
    $.ajax({
        type: 'POST',
        url: '/ServicesCategoryManage/Delete',
        data: { id: id },
        dataType: 'json',
...
function InitSubmitAddForm() {
    $('#form_data_add').on('submit', function (e) {
        ...
        $.ajax({
            url: '/ServicesCategoryManage/P_Add',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
function InitSubmitEditForm() {
    $('#form_data_edit').on('submit', function (e) {
        ...
        let formData = new FormData($formElm[0]);
        $.ajax({
            url: '/ServicesCategoryManage/P_Edit',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
function ChangeStatus(elm, e, id, timer) {
    if ($(elm).data('clicked')) {
        ...
        $.ajax({
            type: 'POST',
            url: '/ServicesCategoryManage/ChangeStatus',
            data: {
                id: id,
