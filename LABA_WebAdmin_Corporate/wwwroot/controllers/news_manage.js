...
const columnTable = function () {
	return [
		...
		{
			data: "title",
			render: (data, type, row, meta) => `<div style="position: relative;">${row.isHot === 1 ? '<span class="badge badge badge-info" style="position: absolute; left: -5px; bottom: 100%; padding-bottom: 1px;">Hot</span>' : ''}<a class="table_a_href text-truncate-4 font-13" onclick="ShowViewModal(this, '${row.id}')" href="javascript:void(0);" title="${data}">${data ?? ''}</a></div>`,
			className: "text-wrap",
		},
		{
			data: "description",
		...
		{
			data: "categoryObj",
			render: (data, type, row, meta) => `<p class="text-truncate-4 mb-0 font-13" title="${data.name}">${data.name ?? ''}</p>`,
...
function GetListDropdownNewsCategory() {
	$.ajax({
		type: 'GET',
		url: '/Common/GetListDropdownNewsCategory',
		data: { typeId: 0 },
		dataType: 'json',
...
function ShowEditModal(elm, id) {
	...
	$.get(`/NewsManage/P_Edit/${id}`).done(function (response) {
...
function ShowViewModal(elm, id) {
	let text = $(elm).html();
	$(elm).attr('disabled', true); $(elm).html(_loadAnimationSmallHtml);
	$.get(`/NewsManage/P_View/${id}`).done(function (response) {
		$(elm).attr('disabled', false); $(elm).html(text);
