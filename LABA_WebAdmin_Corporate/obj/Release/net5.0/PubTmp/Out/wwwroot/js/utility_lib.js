...
var _loadAnimationSmallHtml = '<div class="spinner-border d-flex align-content-end flex-wrap" role="status" style="width:1rem;height:1rem;"><span class="sr-only">Loading...</span></div>',
    _iconEditHtml = '<i class="bx bx-edit text-success"></i>',
    _iconAddHtml = '<i class="fa fa-plus-circle"></i>',
    _iconDeleteHtml = '<i class="bx bx-trash text-danger"></i>',
...
var _languageDataTalbeObj = {
    url: '/assets/plugins/datatable/i18n/vi.json',
};
...
function CheckResponseIsSuccess(data) {
    ...
    switch (parseInt(data.result)) {
        ...
        case 0: type = "warning"; ShowToastNoti(type, 'Cảnh báo', data.error.message); break;
...
//Show message
function ShowToastNoti(type, title, message, position = 'top right') {
    let icon = '';
    switch (type) {
        case 'success': icon = 'bx bx-check'; break;
        case 'warning': icon = 'bx bx-error'; break;
        case 'error': icon = 'bx bx-x'; break;
        case 'info': icon = 'bx bx-info-circle'; break;
        default: break;
    }
    Lobibox.notify(type, {
...
function ToSlugString(str) {
    ...
    var from = "ÁÄÂÀÃÅČÇĆĎÉĚËÈÊẼĔȆĞÍÌÎÏİŇÑÓÖÒÔÕØŘŔŠŞŤÚŮÜÙÛÝŸŽáäâàãåčçćďéěëèêẽĕȇğíìîïıňñóöòôốồõøðřŕšşťúůüùûýÿžþÞĐđßÆa·/_,:;";
    var to = "AAAAAACCCDEEEEEEEEGIIIIINNOOOOOORRSSTUUUUUYYZaaaaaacccdeeeeeeeegiiiiinnooooooooorrsstuuuuuyyzbBDdBAa------";
