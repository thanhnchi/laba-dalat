...
{
	public class ServicesCategoryManageController : BaseController<ServicesCategoryManageController>
	{
		private readonly IS_NewsCategory _s_NewsCategory;

		public ServicesCategoryManageController(IS_NewsCategory newsCategory)
		{
		...
		{
			var res = await _s_NewsCategory.getListNewsCategoryParameter<List<M_NewsCategory>>(_accessToken, CommonConstants.OWNER_FARM_ID, status, CommonConstants.OWNER_SERVICES_TYPE_ID, "createdAt", true);
		...
		public async Task<JsonResult> P_Add(EM_NewsCategory model)
		{
			...
			model.typeId = CommonConstants.OWNER_SERVICES_TYPE_ID;
		...
		public async Task<JsonResult> P_Edit(EM_NewsCategory model)
		{
			...
			model.farmId = CommonConstants.OWNER_FARM_ID;
			model.typeId = model.typeId == 2 ? model.typeId : CommonConstants.OWNER_SERVICES_TYPE_ID;
